...
- **Live crypto prices** — ask what a coin is worth (AVAX, BTC, ETH, SOL and other majors) and it pulls the real spot price from CoinGecko *before* answering, in USD and EUR with the 24h move — so it never invents a plausible-but-wrong number the way a raw language model does. Coins it can't price (e.g. the crew's own $GMC) get an honest "can't pull that one live" instead of a guess. No API key required
