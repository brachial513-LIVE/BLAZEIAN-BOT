...
- **Raid alert overlay** — a clean Sound-Alerts-style banner (`/overlay/raid/NAME`) that drops in when you get raided, naming the raider and their viewer count. Configured entirely from a dashboard panel: an in-app **GIF picker** (Klipy) and **sound browser** (Freesound), or paste any GIF/MP3 URL, plus duration/label/confetti — saved per channel so the OBS URL never changes. CC-BY sounds get an automatic on-screen credit. GIF/sound search needs `KLIPY_API_KEY` / `FREESOUND_API_KEY` set in the environment; the manual URL fields work without either
