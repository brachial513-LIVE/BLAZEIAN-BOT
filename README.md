...
- **Live crypto prices** — ask what a coin is worth (AVAX, BTC, ETH, SOL and other majors, plus **$BLAZE** itself) and it pulls the real price *before* answering, so it never invents a plausible-but-wrong number the way a raw language model does. Major coins come from Coinbase (USD + EUR); on-chain tokens like blaze.stream's own $BLAZE are pinned by contract address and priced from live DEX data (Dexscreener), so it's never confused with the several unrelated coins that share the name. Anything it genuinely can't price (e.g. the crew's own $GMC crowns) gets an honest "can't pull that one live" instead of a guess. No API key required
...
- **Giveaways (spin-the-wheel entries)** — viewers enter with a chat command the streamer names (e.g. `!join`); a second command shows the running count. Each viewer counts once, entries persist across restarts, and the dashboard shows the full copyable entrant list (one name per line) to paste straight into an external spin-the-wheel — plus open/close and reset controls
...
- **Free OBS overlays** — an animated emote wall, a live viewer counter, an animated running/reacting mascot, a live Blaze chat overlay, and a raid alert, all as simple Browser Source URLs
...
- **Raid alert overlay** — a clean Sound-Alerts-style banner (`/overlay/raid/NAME`) that drops in when you get raided, naming the raider and their viewer count. Configured entirely from a dashboard panel: an in-app **GIF picker** (Klipy) and **sound browser** (Freesound), paste any GIF/MP3 URL, or **upload your own sound file** (MP3, ≤25MB — committed to the repo so it survives restarts), plus a fully editable banner text (with `{raider}` / `{count}` tokens), duration, label and confetti — saved per channel so the OBS URL never changes. CC-BY sounds get an automatic on-screen credit. GIF/sound search needs `KLIPY_API_KEY` / `FREESOUND_API_KEY` set in the environment; the manual URL and upload options work without either
- **Editable dashboard entries** — custom commands and stream start/end messages each have an **edit** button that loads the entry back into its form, so you can change and re-save instead of deleting and re-typing
...
Actively developed and running live 24/7 across 75+ Blaze channels, completely free for every streamer who joins.
